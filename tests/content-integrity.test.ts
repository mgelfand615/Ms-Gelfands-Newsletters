...
describe("the immediate-ticket list is attached to its own block", () => {
  ...
  it("carries its own items rather than relying on the heading text", () => {
    expect(block?.items?.map((i) => i.en)).toEqual([
      "Dishonesty",
      "Disrespect",
      "Unsafe body",
    ]);
  ...
  it("has a label for the list", () => {
    expect(block?.itemsLabel?.en).toBe("Immediate ticket");
  });
});
